...
fn parse_csf(source: &str) -> Result<CsfFile, String> {
    ...
    Ok(CsfFile { imports, title, description, component, args, arg_types, variants })
}
...
impl CsfFile {
    fn into_stories(self, path: &Path) -> Result<Vec<Story>, String> {
        ...
        let source = std::fs::read_to_string(&component_path).map_err(|error| {
            format!("{}: could not read component {}: {error}", path.display(), component_path.display())
        })?;
        let kind = component_kind(&component_path);
        let file_id = id_from_filename(path);
        let (default_section, default_title) = split_title(self.title.as_deref().unwrap_or(&human_title(&file_id)));
        let title_has_path = self.title.as_deref().is_some_and(|value| value.contains('/'));
        let section = default_section;
        let title = if title_has_path { default_title } else { self.title.clone().unwrap_or(default_title) };
        ...
        let variants = if self.variants.is_empty() {
            vec![CsfVariant { name: "Default".into(), args: Map::new() }]
        } else {
            self.variants
        };
        let multiple = variants.len() > 1 || variants.first().is_some_and(|variant| !variant.name.eq_ignore_ascii_case("default"));
        let mut out = Vec::with_capacity(variants.len());
        for variant in variants {
            let mut values = self.args.clone();
            for (key, value) in variant.args { values.insert(key, value); }
            ...
            let story_title = if multiple { format!("{} / {}", title, variant.name) } else { title.clone() };
            out.push(Story {
                id: story_id,
                title: story_title,
...
fn resolve_import(imports: &[Import], component: Option<&str>, story_path: &Path) -> Result<Import, String> {
    if imports.is_empty() {
        return Err(format!("{}: story must import an HTML or React component", story_path.display()));
    }
    ...
    if imports.len() == 1 { return Ok(imports[0].clone()); }
    Err(format!("{}: export default.component must match one import", story_path.display()))
}

fn resolve_against_story(story_path: &Path, import: &Path) -> PathBuf {
    if import.is_absolute() { import.to_path_buf() } else { story_path.parent().unwrap_or_else(|| Path::new(".")).join(import) }
}
...
fn parse_imports(source: &str) -> Result<Vec<Import>, String> {
    ...
    for line in source.lines() {
        ...
        if line.starts_with("import ") {
            if let Some(import) = parse_import_line(line) { imports.push(import); }
        }
    }
    Ok(imports)
}
...
fn parse_import_line(line: &str) -> Option<Import> {
    ...
    Some(Import { name, path: PathBuf::from(path) })
}
...
fn parse_named_exports(source: &str) -> Result<Vec<CsfVariant>, String> {
    ...
    while i < source.len() {
        if source[i..].starts_with("export const ") {
            i += "export const ".len();
            let name_end = source[i..].find(|ch: char| !ch.is_ascii_alphanumeric() && ch != '_').map(|rel| i + rel).unwrap_or(source.len());
            let name = source[i..name_end].to_string();
            i = name_end;
            while i < source.len() && bytes[i].is_ascii_whitespace() { i += 1; }
            if i < source.len() && bytes[i] == b'=' { i += 1; }
            while i < source.len() && bytes[i].is_ascii_whitespace() { i += 1; }
            if i >= source.len() { break; }
            let value = if bytes[i] == b'{' {
                let end = matching_brace(source, i).ok_or_else(|| format!("unclosed object for export const {name}"))?;
...
fn matching_brace(source: &str, open: usize) -> Option<usize> {
    let bytes = source.as_bytes();
    if bytes.get(open) != Some(&b'{') && bytes.get(open) != Some(&b'[') { return None; }
    ...
    while i < bytes.len() {
        ...
        match ch {
            ...
            b if b == close_ch => { depth -= 1; if depth == 0 { return Some(i); } }
            _ => {}
        }
        i += 1;
    }
...
struct JsParser<'a> { src: &'a str, i: usize }

impl<'a> JsParser<'a> {
    fn new(src: &'a str) -> Self { Self { src, i: 0 } }
    fn parse_object(&mut self) -> Result<Map<String, Value>, String> {
        self.skip(); self.expect('{')?;
        let mut map = Map::new();
        loop {
            self.skip();
            if self.peek() == Some('}') { self.i += 1; break; }
            let key = self.parse_key()?;
            self.skip(); self.expect(':')?;
            let value = self.parse_value()?;
            if key == "component" {
                if let Value::String(name) = value { map.insert(key, Value::String(name)); }
            } else { map.insert(key, value); }
            self.skip();
            if self.peek() == Some(',') { self.i += 1; continue; }
            if self.peek() == Some('}') { self.i += 1; break; }
    ...
    fn parse_ident(&mut self) -> String {
        ...
        while let Some(ch) = self.peek() {
            if ch.is_ascii_alphanumeric() || ch == '_' || ch == '$' { self.i += ch.len_utf8(); } else { break; }
        }
    ...
    fn parse_string(&mut self) -> Result<String, String> {
        ...
        while let Some(ch) = self.peek() {
            self.i += ch.len_utf8();
            if ch == quote { return Ok(out); }
            if ch == '\\' {
                if let Some(next) = self.peek() { out.push(next); self.i += next.len_utf8(); }
                continue;
            }
            out.push(ch);
        }
    ...
    fn parse_number(&mut self) -> Result<Value, String> {
        ...
        while self.peek().is_some_and(|ch| ch.is_ascii_digit()) { self.i += 1; }
        if self.peek() == Some('.') {
            self.i += 1;
            while self.peek().is_some_and(|ch| ch.is_ascii_digit()) { self.i += 1; }
        }
        let raw = &self.src[start..self.i];
        if let Ok(int) = raw.parse::<i64>() { return Ok(Value::Number(int.into())); }
        raw.parse::<f64>().ok().and_then(serde_json::Number::from_f64).map(Value::Number).ok_or_else(|| format!("invalid number {raw}"))
    }
    fn expect(&mut self, expected: char) -> Result<(), String> {
        self.skip();
        if self.peek() == Some(expected) { self.i += expected.len_utf8(); Ok(()) } else { Err(format!("expected '{expected}'")) }
    }
    fn skip(&mut self) {
        loop {
            while self.peek().is_some_and(|ch| ch.is_whitespace()) { self.i += self.peek().unwrap().len_utf8(); }
            if self.src[self.i..].starts_with("//") {
                if let Some(rel) = self.src[self.i..].find('\n') { self.i += rel + 1; continue; }
                self.i = self.src.len(); break;
            }
            break;
        }
    }
...
fn strip_comments(source: &str) -> String {
    ...
    while i < bytes.len() {
        ...
        if let Some(q) = quote {
            out.push(ch as char);
            if ch == b'\\' && i + 1 < bytes.len() { out.push(bytes[i + 1] as char); i += 2; continue; }
            if ch == q { quote = None; }
            i += 1; continue;
        }
        if ch == b'"' || ch == b'\'' || ch == b'`' { quote = Some(ch); out.push(ch as char); i += 1; continue; }
        ...
        if ch == b'/' && bytes.get(i + 1) == Some(&b'*') {
            ...
            i = (i + 2).min(bytes.len()); continue;
        }
        out.push(ch as char); i += 1;
    }
    out
}

fn string_field(map: &Map<String, Value>, key: &str) -> Option<String> { map.get(key)?.as_str().map(str::to_string) }
fn ident_or_string_field(map: &Map<String, Value>, key: &str) -> Option<String> { string_field(map, key) }
fn object_field(map: &Map<String, Value>, key: &str) -> Option<Map<String, Value>> { map.get(key)?.as_object().cloned() }
fn json_object_to_map(object: Map<String, Value>) -> BTreeMap<String, Value> { object.into_iter().collect() }
fn human_title(id: &str) -> String {
    id.split('-').map(|part| { let mut chars = part.chars(); match chars.next() { Some(first) => first.to_uppercase().collect::<String>() + chars.as_str(), None => String::new() } }).collect::<Vec<_>>().join(" ")
}
fn is_ident_start(ch: char) -> bool { ch.is_ascii_alphabetic() || ch == '_' || ch == '$' }
fn is_generic_import(name: &str) -> bool { matches!(name, "html" | "template" | "markup" | "component" | "default") }
...
export default {
  ...
  argTypes: { variant: { control: 'select', options: ['primary', 'ghost'] }, disabled: { control: 'boolean' } },
};
...
"#;
        ...
        assert_eq!(file.variants[1].args["variant"], Value::String("ghost".into()));
    }
    #[test]
    fn parses_html_import() {
        let src = r#"
import html from './badge.html';
export default { title: 'Badge', component: html, args: { tone: 'neutral', label: 'In review' } };
"#;
        let file = parse_csf(src).unwrap();
